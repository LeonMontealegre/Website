...
let bounds = {l: 0, r: 0.8, b: 0, t: 0.5};
...
class SPH {
    ...
    step() {
        ...
        let newPositions = new Array(this.particles.length).fill(V(0,0));
        for (let i = 0; i < this.particles.length; i++) {
            this.particles[i].vel = this.particles[i].vel.add( this.particles[i].acc.scl(dt) ); // apply exterior forces
            newPositions[i] = this.particles[i].pos.add( this.particles[i].vel.scl(dt) ); // predict position
        }
        ...
        for (let i = 0; i < this.particles.length; i++) {
            ...
            this.particles[i].vel = (newPositions[i].sub(particle.pos)).scl(1.0 / dt);
